...
def bentuk_event(df: pd.DataFrame) -> pd.DataFrame:
    ...
    events = tmp.groupby("GROUP_ID", sort=False).agg(
...
def hitung_ringkasan(events: pd.DataFrame, out_df: pd.DataFrame) -> dict:
    ...
    # ------------------------------------------------------
    # Breakdown bulanan 3 kategori (saling eksklusif, total =
    # 100%): status Dual Cycle diprioritaskan; kalau bukan Dual
    # Cycle baru dilihat CONTAINER_STATUS-nya (Combo / Single).
    # ------------------------------------------------------
    ev["KATEGORI"] = np.where(
        ev["STATUS"] == "Dual Cycle", "Dual Cycle", ev["CONTAINER_STATUS"]
    )

    monthly = ev.groupby("BULAN").agg(
        total_event=("KATEGORI", "count"),
        dual=("KATEGORI", lambda s: int((s == "Dual Cycle").sum())),
        combo=("KATEGORI", lambda s: int((s == "Combo").sum())),
        single=("KATEGORI", lambda s: int((s == "Single").sum())),
    )
...
def build_excel_download(out_df: pd.DataFrame, summary: dict, ambang_combo: float, ambang_dual: float) -> BytesIO:
    ...
    ws["A25"] = "BREAKDOWN BULANAN (EVENT / RITASE) -- 3 KATEGORI (SALING EKSKLUSIF)"
    ws["A25"].font = Font(bold=True)
    header_row = 26
    headers = [
        "Bulan", "Total Event",
        "Dual Cycle", "Combo (Non Dual)", "Single (Non Dual)",
        "% Dual Cycle", "% Combo", "% Single",
    ]
    ...
    for bulan, row in monthly.iterrows():
        ...
        ws.cell(r, 4, int(row["combo"]))
        ws.cell(r, 5, int(row["single"]))
        ws.cell(r, 6, float(row["pct_dual"]))
        ws.cell(r, 6).number_format = "0.0%"
        ws.cell(r, 7, float(row["pct_combo"]))
        ws.cell(r, 7).number_format = "0.0%"
        ws.cell(r, 8, float(row["pct_single"]))
    ...
    ws.cell(total_row, 4, tot_combo)
    ws.cell(total_row, 5, tot_single)
    ws.cell(total_row, 6, (tot_dual / tot_evt) if tot_evt else 0)
    ws.cell(total_row, 6).number_format = "0.0%"
    ws.cell(total_row, 7, (tot_combo / tot_evt) if tot_evt else 0)
    ws.cell(total_row, 7).number_format = "0.0%"
    ws.cell(total_row, 8, (tot_single / tot_evt) if tot_evt else 0)
    ...
    # ---- Stacked bar: breakdown bulanan (3 kategori) ----
    if len(monthly) > 0:
        bar2 = BarChart()
        bar2.type = "col"
        bar2.grouping = "percentStacked"
        bar2.overlap = 100
        bar2.title = "Breakdown Bulanan: Dual Cycle vs Combo vs Single (%)"
        data = Reference(ws, min_col=3, max_col=5, min_row=header_row, max_row=r)
...
if len(monthly) > 0:
    # ------------------------------------------------------
    # Chart: breakdown bulanan 3 kategori (Dual Cycle / Combo /
    # Single) dalam bentuk PERSEN, saling eksklusif -> total
    # tiap bulan selalu 100%.
    # ------------------------------------------------------
    monthly_pct_long = monthly.melt(
        id_vars="Bulan",
        value_vars=["pct_dual", "pct_combo", "pct_single"],
        var_name="Kategori",
        value_name="Persentase",
    )
    monthly_pct_long["Kategori"] = monthly_pct_long["Kategori"].map(
        {"pct_dual": "Dual Cycle", "pct_combo": "Combo (Non Dual)", "pct_single": "Single (Non Dual)"}
    )
    monthly_pct_long["Persentase"] = monthly_pct_long["Persentase"] * 100

    fig_month = px.bar(
        monthly_pct_long, x="Bulan", y="Persentase", color="Kategori", barmode="stack",
        title="Breakdown Bulanan: Dual Cycle vs Combo vs Single (%)",
        color_discrete_map={
            "Dual Cycle": "#2E86AB",
            "Combo (Non Dual)": "#F4A261",
            "Single (Non Dual)": "#E76F51",
        },
        text_auto=".1f",
    )
    fig_month.update_layout(yaxis=dict(title="% dari Total Event", range=[0, 100]))
    st.plotly_chart(fig_month, width='stretch')

    with st.expander("📋 Tabel Breakdown Bulanan (jumlah & persen)"):
        tabel_bulan = monthly[
            ["Bulan", "total_event", "dual", "combo", "single", "pct_dual", "pct_combo", "pct_single"]
        ].copy()
        tabel_bulan.columns = [
            "Bulan", "Total Event", "Dual Cycle", "Combo", "Single",
            "% Dual Cycle", "% Combo", "% Single",
        ]
        for c in ["% Dual Cycle", "% Combo", "% Single"]:
