"""
================================================================
DASHBOARD ANALISIS DUAL CYCLE
Port dari logika VBA (AnalisisDualCycle) ke Python + Streamlit
================================================================

Konsep (sama seperti VBA):

LAYER 1 - COMBO / SINGLE (per truk / CAR_CHE_ID)
    - Hanya kontainer size 20 ft yang jadi kandidat.
    - Truk sama, aktivitas sama (LOAD-LOAD atau DISC-DISC).
    - Gap (jarak waktu, dihitung dari kolom G maupun H, ambil
      yang terkecil) <= AMBANG_COMBO_MENIT -> dipasangkan jadi
      "Combo". Sisanya "Single".
    - Semua kandidat pasangan dikumpulkan dulu, diurutkan dari
      gap terkecil, lalu dipasangkan greedy (1 baris cuma boleh
      dipakai sekali).

BENTUK EVENT
    - Tiap grup Combo/Single jadi satu "event" (ritase).
    - start/end waktu event diambil dari kombinasi kolom G & H,
      tergantung aktivitasnya DISC atau LOAD.

LAYER 2 - DUAL CYCLE (per truk, antar EVENT)
    - Truk sama, aktivitas beda (satu LOAD satu DISC).
    - Gap dihitung simetris & toleran overlap (lihat
      hitung_gap_event).
    - Gap <= AMBANG_DUAL_MENIT -> greedy matching dari gap
      terkecil -> status "Dual Cycle". Sisanya "Non Dual".

EVENT_ID diberi nomor urut GLOBAL, mengikuti urutan kemunculan
truk pertama kali di data asli, dan di dalam truk diurutkan
berdasarkan waktu mulai event.
================================================================
"""
