...
def hitung_ringkasan(events: pd.DataFrame, out_df: pd.DataFrame) -> dict:
    ...
    # ------------------------------------------------------
    # Breakdown bulanan PERSIS spt VBA: 2 kategori saja per
    # event/ritase -- Dual Cycle vs Non Dual (TIDAK dipecah lagi
    # jadi Combo/Single seperti versi sebelumnya, karena VBA
    # aslinya juga cuma breakdown Dual Cycle vs Non Dual per bulan).
    # ------------------------------------------------------
...
def build_excel_download(out_df: pd.DataFrame, summary: dict, ambang_combo: float, ambang_dual: float) -> BytesIO:
    ...
    header_row = 26
    headers = ["Bulan", "Total Event", "Dual Cycle", "Non Dual", "% Dual Cycle"]
...
if len(monthly) > 0:
    # ------------------------------------------------------
    # Chart: breakdown bulanan berdasarkan AKTIVITAS Dual Cycle
    # vs Non Dual (jumlah event/ritase per bulan) -- persis spt
    # chart xlColumnStacked di VBA (kolom C=Dual Cycle, D=Non Dual).
    # ------------------------------------------------------
    monthly_long = monthly.melt(
        id_vars="Bulan",
        value_vars=["dual", "non_dual"],
        var_name="Kategori",
        value_name="Jumlah Event",
    )
    monthly_long["Kategori"] = monthly_long["Kategori"].map(
        {"dual": "Dual Cycle", "non_dual": "Non Dual"}
    )

    fig_month = px.bar(
        monthly_long, x="Bulan", y="Jumlah Event", color="Kategori", barmode="stack",
    ...
    with st.expander("📋 Tabel Breakdown Bulanan (jumlah & persen)"):
        tabel_bulan = monthly[["Bulan", "total_event", "dual", "non_dual", "pct_dual"]].copy()
        tabel_bulan.columns = ["Bulan", "Total Event", "Dual Cycle", "Non Dual", "% Dual Cycle"]
        tabel_bulan["% Dual Cycle"] = (tabel_bulan["% Dual Cycle"] * 100).round(1).astype(str) + "%"
